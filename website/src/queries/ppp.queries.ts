...
import type { PPPProfileCreateInput, PPPProfileUpdateInput, PPPSecretUpdateInput } from '@/types/ppp'
...
// ── PPP Secret mutations ─────────────────────────────────────────────────

export function useUpdatePPPSecretMutation(deviceId: MaybeRefOrGetter<string | null>) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: PPPSecretUpdateInput }) =>
      pppService.updateSecret(String(toValue(deviceId)), id, input),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.ppp.secrets(String(toValue(deviceId))) })
    },
  })
}

export function useRemovePPPSecretMutation(deviceId: MaybeRefOrGetter<string | null>) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => pppService.removeSecret(String(toValue(deviceId)), id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.ppp.secrets(String(toValue(deviceId))) })
    },
  })
