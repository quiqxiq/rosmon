...
// (lewat form publik) yang masuk antrian admin. PPPProfileID/DeviceID = hint
// paket/area yang diminta (opsional); detail provisioning final diisi operator
// saat complete-install. Status: 'pending'|'approved'|'rejected'|'cancelled'.
type CustomerRegistration struct {
	...
	PPPProfileID *uint `gorm:"index"` // paket yang diminta (hint)
